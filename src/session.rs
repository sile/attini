...
pub enum SessionRecord {
    ...
    /// A plan file was generated by a `submit_plan` terminal tool call
    /// during `plan create`.
    PlanCreated {
        ts: u64,
        path: String,
    ...
    /// A sealed plan was approved and executed by `plan run`.
    PlanApproved {
        ts: u64,
        path: String,
        plan_sha256: String,
    },
    /// A sealed plan was delegated to a synchronous child subagent.
    PlanDelegated {
        ts: u64,
...
pub struct AutoDecidedBy {
    ...
    pub reason: String,
    /// Present when the approval came from an approved plan action.
...
impl DisplayJson for AutoDecidedBy {
    fn fmt(&self, f: &mut JsonFormatter<'_, '_>) -> std::fmt::Result {
        f.object(|f| {
            ...
            if let Some(h) = &self.plan_sha256 {
                f.member("plan_sha256", h)?;
            }
            if let Some(id) = &self.action_id {
                f.member("action_id", id)?;
            }
            Ok(())
        })
    }
    ...
    fn fmt(&self, f: &mut JsonFormatter<'_, '_>) -> std::fmt::Result {
        match self {
            ...
            Self::PlanCreated {
                ...
                f.member("kind", "plan_created")?;
                f.member("ts", ts)?;
                f.member("path", path)?;
                f.member("plan_sha256", plan_sha256)
            }),
            Self::PlanApproved {
                ts,
                path,
            ...
            Self::PlanDelegated {
                ...
                f.member("kind", "plan_delegated")?;
                f.member("ts", ts)?;
                f.member("parent_session", parent_session)?;
                f.member("plan_sha256", plan_sha256)
            }),
        }
    }
...
fn parse_session_record_line(line: &str) -> Result<Option<SessionRecord>, String> {
    ...
    match kind.as_str() {
        ...
        "plan_created" => {
            let ts = read_u64(value, "ts")?;
            let path = read_string(value, "path")?;
            let plan_sha256 = read_string(value, "plan_sha256")?;
            Ok(Some(SessionRecord::PlanCreated {
                ts,
                path,
                plan_sha256,
            }))
        ...
        "plan_approved" => {
            ...
            let path = read_string(value, "path")?;
            let plan_sha256 = read_string(value, "plan_sha256")?;
            Ok(Some(SessionRecord::PlanApproved {
                ts,
                path,
                plan_sha256,
            }))
        ...
        "plan_delegated" => {
            let ts = read_u64(value, "ts")?;
            let parent_session = read_string(value, "parent_session")?;
            let plan_sha256 = read_string(value, "plan_sha256")?;
            Ok(Some(SessionRecord::PlanDelegated {
                ts,
                parent_session,
                plan_sha256,
            }))
...
fn parse_auto_decided_by(
    ...
    let plan_sha256 = match m
        .to_member("plan_sha256")
        .map_err(|e| e.to_string())?
        .optional()
    {
        Some(v) if v.as_raw_str().trim() != "null" => Some(read_string(m, "plan_sha256")?),
        _ => None,
    };
    let action_id = match m
        .to_member("action_id")
        .map_err(|e| e.to_string())?
        .optional()
    {
        Some(v) if v.as_raw_str().trim() != "null" => Some(read_string(m, "action_id")?),
        _ => None,
    };
